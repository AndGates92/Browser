...
void command_menu::CommandMenu::setProperties() {
	...
	// Selection behaviour
	QINFO_PRINT(global_types::qinfo_level_e::ZERO, commandMenuSettings,  "Selection behaviour: " << command_menu::selection);
...
QRect command_menu::CommandMenu::viewportRectangle(const int row) const {
	...
	if (currentRect.isEmpty()) {
		...
		QINFO_PRINT(global_types::qinfo_level_e::ZERO, commandMenuViewport,  "Viewport coordinates: x " << x << " y " << y);
		QINFO_PRINT(global_types::qinfo_level_e::ZERO, commandMenuViewport,  "Viewport dimension: width " << width << " height " << height);
	}
...
void command_menu::CommandMenu::scrollTo(const QModelIndex & index, const QAbstractItemView::ScrollHint hint) {
	...
	QINFO_PRINT(global_types::qinfo_level_e::ZERO, commandMenuScrollBar,  "Scrollbar values: vertical " << vValue << " horizontal " << hValue);
...
QModelIndex command_menu::CommandMenu::moveCursor(const QAbstractItemView::CursorAction cursorAction, const Qt::KeyboardModifiers modifiers) {
	...
	QINFO_PRINT(global_types::qinfo_level_e::ZERO, commandMenuCursor,  "Current index is set at row " << currIndex.row() << " and column " << currIndex.column());
...
void command_menu::CommandMenu::setSelection(const QRect & rect, QItemSelectionModel::SelectionFlags command) {
	...
	QINFO_PRINT(global_types::qinfo_level_e::ZERO, commandMenuSelection,  "Selection boundaries: top row " << topRowSel << " bottom row " << bottomRowSel);
