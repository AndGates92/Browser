...
action::Action::Action(QObject * parent, const QString & text, const QIcon & icon) : QAction(icon, text, parent) {
	...
	QINFO_PRINT(global_types::qinfo_level_e::ZERO, actionOverall,  "Creating open popup");

}

action::Action::~Action() {
	QINFO_PRINT(global_types::qinfo_level_e::ZERO, actionOverall,  "Destructor of open popup class");

}

void action::Action::setShortcut(const key_sequence::KeySequence & key) {

	QINFO_PRINT(global_types::qinfo_level_e::ZERO, actionShortcut,  "Setting primary shortcut: " << key.toString());
...
void action::Action::setShortcuts(const std::list<key_sequence::KeySequence> & keys) {
	...
	for (std::list<key_sequence::KeySequence>::const_iterator key = keys.cbegin(); key != keys.cend(); key++) {
		QINFO_PRINT(global_types::qinfo_level_e::ZERO, actionShortcut,  "Appending shortcut " << key->toString());
...
key_sequence::KeySequence action::Action::shortcut() const {
	...
	QINFO_PRINT(global_types::qinfo_level_e::ZERO, actionShortcut,  "Setting primary shortcut: " << primaryKey.toString());
