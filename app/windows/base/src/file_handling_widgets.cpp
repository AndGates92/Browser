...
#include "windows/base/include/file_handling_widgets.h"
...
window::FileHandlingWidgets::FileHandlingWidgets(QWidget *widgetParent) :
	...
	fileView(Q_NULLPTR)
{
	...
	this->applyAction = std::move(secondary_window::createAction(widgetParent, std::string(), std::string(), key_sequence::KeySequence(Qt::Key_unknown)));
	this->browseAction = std::move(secondary_window::createAction(widgetParent, "Browse", "Browse files", key_sequence::KeySequence(Qt::Key_B)));
	this->cancelAction = std::move(secondary_window::createAction(widgetParent, "Cancel", "Cancel operation", key_sequence::KeySequence(Qt::Key_Escape)));
	this->typeAction = std::move(secondary_window::createAction(widgetParent, "Insert", "Insert", key_sequence::KeySequence(Qt::Key_I)));

	this->pathToOpen = secondary_window::createLineEdit(widgetParent, std::string(), this->typeAction);
...
//	const QStringList filters({"text/html", "application/octet-stream"});
	...
	this->fileView = std::move(secondary_window::createFileView(this->fileModel, widgetParent, filters, QDir::currentPath()));

}

window::FileHandlingWidgets::~FileHandlingWidgets() {
	LOG_INFO(logger::info_level_e::ZERO, fileHandlingWidgetsOverall,  "Destructor of FileHandlingWidgets class");
}

const QString window::FileHandlingWidgets::getPathFromModelIndex(const std::unique_ptr<QFileSystemModel> & model, const QModelIndex & index) {
...
void window::FileHandlingWidgets::fileViewClickAction(const QModelIndex & index) {
...
void window::FileHandlingWidgets::fileViewDoubleClickAction(const QModelIndex & index) {
	const QString path(this->getPathFromModelIndex(this->fileModel, index));
...
void window::FileHandlingWidgets::directoryLoadedAction(const QString & path) {
...
void window::FileHandlingWidgets::addActionsToWidget(QWidget * widget) {
