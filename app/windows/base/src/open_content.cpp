...
window::OpenContent::OpenContent(QWidget *widgetParent) : window::FileHandlingWidgets(widgetParent) {
...
void window::OpenContent::openPath() {
...
void window::OpenContent::doubleClickOnFile(const QFileInfo & pathInfo) {
	this->pathToOpen->setText(pathInfo.absoluteFilePath());
	this->openPath();
}

CONST_GETTER(window::OpenContent::getTypedPath, QString, this->pathToOpen->text())
