...
main_window::CtrlTab::CtrlTab(QWidget * parent, const std::shared_ptr<main_window::Core> & core) : main_window::CtrlBase(parent, core, main_window::commandFileFullPath), findSettings(QString(), global_enums::offset_type_e::IDLE, false, false) {
...
main_window::CtrlTab::~CtrlTab() {

}

void main_window::CtrlTab::createExtraShortcuts() {
	LOG_INFO(logger::info_level_e::ZERO, mainWindowCtrlTabOverall,  "Create shortcuts");

}

void main_window::CtrlTab::connectExtraSignals() {
	LOG_INFO(logger::info_level_e::ZERO, mainWindowCtrlTabOverall,  "Connect signals");

	// open tab action (from fileMenu)
	connect(this->core->topMenuBar->getFileMenu()->openTabAction.get(), &QAction::triggered, this, &main_window::CtrlTab::setUpOpenNewTab);

	std::unique_ptr<main_window::TabWidget> & tabs = this->core->tabs;

	// When the file has been read, then show it on the screen
	connect(this->core->topMenuBar->getFileMenu().get(), &file_menu::FileMenu::updateCenterWindowSignal, tabs.get(), &main_window::TabWidget::openFileInCurrentTab);
	connect(this->core->popup->getOpenFilePopup().get(), &open_popup::OpenPopup::fileRead, tabs.get(), &main_window::TabWidget::openFileInCurrentTab);

	// Search text in webpage
	connect(this->core->topMenuBar->getEditMenu().get(), &edit_menu::EditMenu::triggerSearch, this, &main_window::CtrlTab::setUpSearchFromMenu);

	std::unique_ptr<main_window::StatusBar> & statusBar = this->core->bottomStatusBar;
	connect(this, &main_window::CtrlTab::currentTabSrcChanged, statusBar.get(), &main_window::StatusBar::setContentPathText);

	// Updates to the window depending on changes in tabs
	connect(tabs.get(), &main_window::TabWidget::tabSourceChanged, this, &main_window::CtrlTab::createContentPathTextFromSource);

	connect(tabs.get(), &main_window::TabWidget::currentChanged, this, &main_window::CtrlTab::updateStatusBar);
	connect(tabs.get(), &main_window::TabWidget::numberTabsChanged, this, &main_window::CtrlTab::updateStatusBar);

	// Progress bar connections
	connect(tabs.get(), &main_window::TabWidget::tabNearlyDisconnected, this, &main_window::CtrlTab::disconnectTab);
	connect(tabs.get(), &main_window::TabWidget::tabNearlyConnected, this, &main_window::CtrlTab::connectTab);

	// Update info bar
	connect(tabs.get(), &main_window::TabWidget::tabCloseRequested, this, &main_window::CtrlTab::updateStatusBar);

	// Search text in tab
	connect(tabs.get(), &main_window::TabWidget::searchResultChanged, this, &main_window::CtrlTab::printSearchResult);
	connect(tabs.get(), &main_window::TabWidget::findTextFinished, this, &main_window::CtrlTab::processSearchReturnValue);

	// History of the tab
	connect(tabs.get(), &main_window::TabWidget::historyItemChanged, this, &main_window::CtrlTab::historyBoundaryHit);
...
//************************************************************************************
void main_window::CtrlTab::setUpOpenNewTab() {
	LOG_INFO(logger::info_level_e::ZERO, mainWindowCtrlTabSearch,  "Open new tab");
	const main_window::state_e requestedWindowState = main_window::state_e::OPEN_TAB;
	this->changeWindowState(requestedWindowState, main_window::state_postprocessing_e::POSTPROCESS);
}
...
//************************************************************************************
void main_window::CtrlTab::closeTab(const int & index) {
	LOG_INFO(logger::info_level_e::ZERO, mainWindowCtrlTabTabs,  "Close tab " << index);
	this->core->tabs->removeTab(index);
}

void main_window::CtrlTab::addNewTabAndSearch(const QString & search) {
	const int index = this->addNewTab(main_window::page_type_e::UNKNOWN, nullptr);
	this->searchTab(index, search);
}

int main_window::CtrlTab::addNewTab(const main_window::page_type_e & type, const void * data) {
...
void main_window::CtrlTab::searchTab(const int & index, const QString & search) {
	LOG_INFO(logger::info_level_e::ZERO, mainWindowCtrlTabSearch,  "User input " << search << " in tab " << index);
	main_window::page_type_e type = this->core->tabs->getPageType(index);

	if (type == main_window::page_type_e::UNKNOWN) {
		if (main_window::isFile(search) == true) {
			type = main_window::page_type_e::TEXT;
		} else if ((main_window::isUrl(search) == true) || (main_window::isText(search) == true)) {
			type = main_window::page_type_e::WEB_CONTENT;
		} else {
...
void main_window::CtrlTab::searchCurrentTab(const QString & search) {
	...
	switch (windowState) {
		case main_window::state_e::NEW_SEARCH:
		case main_window::state_e::EDIT_SEARCH:
			LOG_INFO(logger::info_level_e::ZERO, mainWindowCtrlTabTabs,  "Search " << search << " in tab " << tabIndex);
			this->searchTab(tabIndex, search);
			break;
		case main_window::state_e::FIND:
		case main_window::state_e::FIND_DOWN:
		case main_window::state_e::FIND_UP:
...
void main_window::CtrlTab::scrollTab(const global_enums::offset_type_e direction) {
	const int tabIndex = this->core->getCurrentTabIndex();
	this->core->tabs->scrollTab(tabIndex, direction);
}

void main_window::CtrlTab::goToPageInHistory(const main_window::navigation_type_e direction) {
	const int tabIndex = this->core->getCurrentTabIndex();
	this->core->tabs->goToHistoryItem(tabIndex, direction);
}

void main_window::CtrlTab::extractContentPath(const int & index) {

	const int tabCount = this->core->getTabCount();

	if (tabCount > 0) {
		const main_window::page_type_e tabType = this->core->tabs->getPageType(index);
...
void main_window::CtrlTab::reloadTab(const int & tabIndex) {
	this->core->tabs->reloadTabContent(tabIndex);
}

void main_window::CtrlTab::moveTab(const int & tabIndex) {
...
void main_window::CtrlTab::moveCursor(const int & tabIndex) {
...
void main_window::CtrlTab::connectTab(const int & tabIndex) {
	const main_window::page_type_e tabType = this->core->tabs->getPageType(tabIndex);
	...
	std::shared_ptr<main_window::Tab> tab = this->core->tabs->widget(tabIndex);

	std::unique_ptr<main_window::StatusBar> & statusBar = this->core->bottomStatusBar;
	connect(tab.get(), &main_window::Tab::verticalScrollChanged, statusBar.get(), &main_window::StatusBar::setVScroll);
	statusBar->setVScroll(tab->getVerticalScroll());

	connect(tab.get(), &main_window::Tab::loadProgressChanged, statusBar.get(), &main_window::StatusBar::setProgressValue);
...
void main_window::CtrlTab::disconnectTab(const int & tabIndex) {
	const main_window::page_type_e tabType = this->core->tabs->getPageType(tabIndex);
	...
	const std::shared_ptr<main_window::Tab> tab = this->core->tabs->widget(tabIndex);

	std::unique_ptr<main_window::StatusBar> & statusBar = this->core->bottomStatusBar;
	disconnect(tab.get(), &main_window::Tab::verticalScrollChanged, statusBar.get(), &main_window::StatusBar::setVScroll);
	disconnect(tab.get(), &main_window::Tab::loadProgressChanged, statusBar.get(), &main_window::StatusBar::setProgressValue);
...
void main_window::CtrlTab::executeActionOnOffset(const int & offset) {
	...
	if (windowState == main_window::state_e::MOVE_RIGHT) {
		sign = global_enums::sign_e::PLUS;
	} else if (windowState == main_window::state_e::MOVE_LEFT) {
		sign = global_enums::sign_e::MINUS;
	} else if (windowState == main_window::state_e::MOVE_TAB) {
...
void main_window::CtrlTab::executeActionOnTab(const int & index) {
	...
	if ((tabCount > tabIndex) && (tabIndex >= 0) && (tabCount > 0)) {
		switch (windowState) {
			case main_window::state_e::CLOSE_TAB:
				this->closeTab(tabIndex);
				break;
			case main_window::state_e::MOVE_TAB:
				this->convertToAbsTabIndex(tabIndex, global_enums::sign_e::NOSIGN);
				break;
			case main_window::state_e::REFRESH_TAB:
...
void main_window::CtrlTab::executeTabAction(const int & userInput) {
	const main_window::state_e windowState = this->core->getMainWindowState();
	const global_enums::offset_type_e offsetType = this->core->getOffsetType();
	...
	switch (windowState) {
		...
		case main_window::state_e::CLOSE_TAB:
			...
			break;
		case main_window::state_e::MOVE_RIGHT:
		case main_window::state_e::MOVE_LEFT:
...
void main_window::CtrlTab::processTabIndex(const QString & userInputStr) {
...
void main_window::CtrlTab::actionOnReleasedKey(const main_window::state_e & windowState, QKeyEvent * event) {
...
void main_window::CtrlTab::executeAction(const main_window::state_e & windowState) {
	const QString userTypedText = this->core->getUserText();

	switch (windowState) {
		case main_window::state_e::OPEN_TAB:
		...
		case main_window::state_e::FIND_UP:
			...
			this->changeWindowState(main_window::state_e::IDLE, main_window::state_postprocessing_e::POSTPROCESS);
			break;
		case main_window::state_e::SCROLL_UP:
			this->scrollTab(global_enums::offset_type_e::UP);
			this->changeWindowState(main_window::state_e::IDLE, main_window::state_postprocessing_e::POSTPROCESS);
			break;
		case main_window::state_e::SCROLL_DOWN:
...
void main_window::CtrlTab::prepareAction(const main_window::state_e & windowState, QKeyEvent * event) {
	...
	switch (windowState) {
		case main_window::state_e::COMMAND:
			if (pressedKey == Qt::Key_Space) {
				this->executeCommand(userTypedText, main_window::state_postprocessing_e::NONE);
			}
			break;
		case main_window::state_e::OPEN_TAB:
		case main_window::state_e::NEW_SEARCH:
...
void main_window::CtrlTab::convertToAbsTabIndex(const int & offset, const global_enums::sign_e & sign) {

	const main_window::state_e windowState = this->core->getMainWindowState();

	int distance = 0;
	// offset is main_window::emptyUserInput if the argument is not passed
...
void main_window::CtrlTab::postprocessWindowStateChange(const main_window::state_e & previousState) {
	const main_window::state_e windowState = this->core->getMainWindowState();
	...
	const bool isFindState = ((windowState == main_window::state_e::FIND) || (windowState == main_window::state_e::FIND_DOWN) || (windowState == main_window::state_e::FIND_UP));
	std::unique_ptr<main_window::StatusBar> & statusBar = this->core->bottomStatusBar;
	statusBar->showSearchResult(isFindState);

	// If requesting to go to the idle state, enable shortcuts
	switch (windowState) {
		case main_window::state_e::OPEN_FILE:
		...
		case main_window::state_e::EDIT_SEARCH:
			...
			this->printUserInput(main_window::text_action_e::SET, searchText);
			this->setAllShortcutEnabledProperty(false);
			this->setFocus();
			break;
		case main_window::state_e::FIND_DOWN:
		...
		case main_window::state_e::SCROLL_DOWN:
			...
			this->changeWindowState(main_window::state_e::IDLE, main_window::state_postprocessing_e::POSTPROCESS);
			break;
		case main_window::state_e::HISTORY_PREV:
			this->goToPageInHistory(main_window::navigation_type_e::PREVIOUS);
			this->changeWindowState(main_window::state_e::IDLE, main_window::state_postprocessing_e::POSTPROCESS);
			break;
		case main_window::state_e::HISTORY_NEXT:
			this->goToPageInHistory(main_window::navigation_type_e::NEXT);
			this->changeWindowState(main_window::state_e::IDLE, main_window::state_postprocessing_e::POSTPROCESS);
			break;
		default:
			this->setAllShortcutEnabledProperty(false);
...
bool main_window::CtrlTab::isValidWindowState(const main_window::state_e & requestedWindowState) {
	bool isValid = false;
	const main_window::state_e windowState = this->core->getMainWindowState();
	const int tabCount = this->core->getTabCount();
	...
	switch (requestedWindowState) {
		...
		case main_window::state_e::HISTORY_NEXT:
			...
			isValid = ((tabCount > 0) && (windowState == main_window::state_e::IDLE));
			break;
		case main_window::state_e::MOVE_RIGHT:
		case main_window::state_e::MOVE_LEFT:
		case main_window::state_e::MOVE_TAB:
			...
			isValid = ((tabCount > 1) && (windowState == main_window::state_e::IDLE));
			break;
		case main_window::state_e::COMMAND:
...
void main_window::CtrlTab::updateStatusBar(const int & tabIndex) {
	this->updateInfo(tabIndex);
	this->updateScroll();
	this->extractContentPath(tabIndex);
}

void main_window::CtrlTab::printSearchResult(const main_window::search_data_s & data) const {
	std::unique_ptr<main_window::StatusBar> & statusBar = this->core->bottomStatusBar;
...
void main_window::CtrlTab::historyBoundaryHit(const global_enums::element_position_e & position) {

	if ((position == global_enums::element_position_e::BEGINNING) || (position == global_enums::element_position_e::END)) {
		std::shared_ptr<main_window::PopupContainer> container = this->core->popup;
...
void main_window::CtrlTab::processSearchReturnValue(const bool & found) {
...
void main_window::CtrlTab::createContentPathTextFromSource(const main_window::page_type_e & type, const QString & source) {
	...
	switch (type) {
		...
		case main_window::page_type_e::TEXT:
			...
			text = QString(main_window::filePrefix) + source;
			break;
...
void main_window::CtrlTab::createOpenPrompt() {
	std::shared_ptr<main_window::PopupContainer> container = this->core->popup;
...
void main_window::CtrlTab::setUpSearchFromMenu(const find_settings::FindSettings & settings) {
	this->findSettings = settings;

	main_window::state_e windowState = main_window::state_e::IDLE;
	...
	if (searchDirection == global_enums::offset_type_e::DOWN) {
		windowState = main_window::state_e::FIND_UP;
	} else if (searchDirection == global_enums::offset_type_e::UP) {
		windowState = main_window::state_e::FIND_DOWN;
	}
	this->changeWindowState(windowState, main_window::state_postprocessing_e::POSTPROCESS);
}
