...
json_parser::JsonParser::JsonParser(QString fileName, QIODevice::OpenModeFlag openFlags) : json_wrapper::JsonWrapper::JsonWrapper(fileName,openFlags) {
	...
	LOG_INFO(logger::info_level_e::ZERO, jsonParserOverall,  "Creating JSON Parser of file " << fileName);
...
json_parser::JsonParser::JsonParser(const json_parser::JsonParser & rhs) : json_wrapper::JsonWrapper(rhs) {
	...
	LOG_INFO(logger::info_level_e::ZERO, jsonParserOverall,  "Copy constructor json parser");

}

json_parser::JsonParser & json_parser::JsonParser::operator=(const json_parser::JsonParser & rhs) {

	LOG_INFO(logger::info_level_e::ZERO, jsonParserOverall,  "Copy assignment operator for json parser");
...
json_parser::JsonParser::JsonParser(json_parser::JsonParser && rhs) : json_wrapper::JsonWrapper(std::move(rhs)) {
	...
	LOG_INFO(logger::info_level_e::ZERO, jsonParserOverall,  "Move constructor json parser");
}

json_parser::JsonParser & json_parser::JsonParser::operator=(json_parser::JsonParser && rhs) {

	LOG_INFO(logger::info_level_e::ZERO, jsonParserOverall,  "Move assignment operator for json parser");
...
json_parser::JsonParser::~JsonParser() {
	LOG_INFO(logger::info_level_e::ZERO, jsonParserOverall,  "Destructor of JsonParser class");
...
const QString json_parser::JsonParser::findKeyValue(const QString & treeRoot, const QString & key) const {
	...
	LOG_INFO(logger::info_level_e::ZERO, jsonParserFileContent, "JSON tree root: " << treeRoot << " key: " << key << " value " << foundValue);
...
QMap<QString, QString> json_parser::JsonParser::findKeyAllValues(const QString & key) const {
	...
	if (this->jsonContent.type() == QJsonValue::Object) {
		...
		for (QStringList::const_iterator keyIter = jsonKeys.cbegin(); keyIter != jsonKeys.cend(); keyIter++) {
			LOG_INFO(logger::info_level_e::ZERO, jsonParserFileContent, "Searching key: " << key << " under tree " << *keyIter);
...
QString json_parser::JsonParser::searchJson(const QJsonValue & content, const QString & key) const {
	...
	switch (content.type()) {
		...
		{
			LOG_INFO(logger::info_level_e::ZERO, jsonParserFileContent, "Searching in JSON Object");
		...
		{
			LOG_INFO(logger::info_level_e::ZERO, jsonParserFileContent, "Searching in JSON Array");
		...
		default:
			LOG_INFO(logger::info_level_e::ZERO, jsonParserFileContent, "Cannot find key in type " << content.type() << ". Expected values are array (" << QJsonValue::Array << ") or object (" << QJsonValue::Object << ")");
			break;
	}
...
QString json_parser::JsonParser::searchJsonObject(const QJsonObject & object, const QString & key) const {
	...
	if (object.contains(key) == true) {
		...
		LOG_INFO(logger::info_level_e::ZERO, jsonParserValue, "Found key " << key << " with value " << valueStr);
	} else {
		const QStringList jsonKeys (object.keys());
		// Iterate over all key of the object
		for (QStringList::const_iterator keyIter = jsonKeys.cbegin(); keyIter != jsonKeys.cend(); keyIter++) {
			LOG_INFO(logger::info_level_e::ZERO, jsonParserFileContent, "JSON key: " << *keyIter);
