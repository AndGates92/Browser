...
main_window_json_data::MainWindowJsonData & main_window_json_data::MainWindowJsonData::operator=(main_window_json_data::MainWindowJsonData && rhs) {
	...
	if (&rhs != this) {
		...
		this->shortcut = std::exchange(rhs.shortcut, Qt::Key_unknown);
...
std::string main_window_json_data::MainWindowJsonData::print() const {
	...
	structInfo = structInfo + "- shortcut key for the action: " + global_functions::qEnumToQString(this->shortcut, true).toStdString() + "\n";
...
BASE_GETTER(main_window_json_data::MainWindowJsonData::getShortcut, Qt::Key, this->shortcut)
