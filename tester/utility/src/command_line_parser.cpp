...
Q_LOGGING_CATEGORY(commandLineParserOverall, "commandLineParser.overall", MSG_TYPE_LEVEL)
...
command_line_parser::CommandLineParser::CommandLineParser(int & argc, char** argv) : json_action::JsonAction<command_line_argument::CommandLineArgument>(QString::fromStdString(command_line_parser::testerJsonFullPath)), argc(argc), argv(argv), decodedArguments(command_line_parser::CommandLineParser::argument_map_t()) {
	QINFO_PRINT(global_enums::qinfo_level_e::ZERO, commandLineParserOverall,  "Creating command line parser\n" << *this);
...
command_line_parser::CommandLineParser::~CommandLineParser() {
	QINFO_PRINT(global_enums::qinfo_level_e::ZERO, commandLineParserOverall,  "Command line parser destructor");
}
...
void command_line_parser::CommandLineParser::extractArguments() {
	...
	while (counter < this->argc) {
		...
		if (numberOfArguments == 0) {
			...
			QEXCEPTION_ACTION_COND(((counter + numberOfArguments) >= this->argc), throw, "Argument " << option << " expects " << numberOfArguments << " arguments (range within the command line from " << counter << " to " << (counter + numberOfArguments) << " but the number of provided arguments is " << this->argc);
