...
compile_binary() {
	...
	if [ ${compile} -eq 1 ]; then
		...
		echotimestamp " ========================================================================="
		(set -x; \
		 make ${target} LOG_DIR=${LOGDIR} QTLOGFILENAME=${EXEQTLOG} LOGFILENAME=${EXELOG} PROJ_NAME=${PROJNAME} EXE_NAME=${EXENAME} TESTER_EXE_NAME=${TESTEREXENAME} BIN_DIR=${EXEDIR} QINFO_VERBOSITY=${QINFO_VERBOSITY} INFO_VERBOSITY=$(INFO_VERBOSITY) CEXTRAFLAGS=${CEXTRAFLAGS} SANITIZER=${SANITIZER} COVERAGE=${COVERAGE} PROFILER=${PROFILER} 1> ${LOGDIR}/${COMPLOG} 2> ${LOGDIR}/${COMPERR} COMPILE_TYPE=${COMPILETYPE})
...
if [ ${tests} -eq 1 ]; then
	...
	echotimestamp " ${ITEMSYMBOL} Executable Qt logfile: ${EXEQTLOG}"
fi
...
if [ ${doc} -eq 1 ]; then
	...
	echotimestamp " ========================================================================="
	(set -x; \
	 make doc LOG_DIR=${LOGDIR} QTLOGFILENAME=${EXEQTLOG} LOGFILENAME=${EXELOG} PROJ_NAME=${PROJNAME} EXE_NAME=${EXENAME} TESTER_EXE_NAME=${TESTEREXENAME} BIN_DIR=${EXEDIR} CEXTRAFLAGS=${CEXTRAFLAGS} 1> ${LOGDIR}/${DOCLOG} 2> ${LOGDIR}/${DOCERR})
fi
...
if [ ${memleak} -eq 1 ]; then
	...
	(set -x; \
	 make memleak LOG_DIR=${LOGDIR} LOGFILENAME=${EXEVALGRINDLOG} PROJ_NAME=${PROJNAME} EXE_NAME=${EXENAME} TESTER_EXE_NAME=${TESTEREXENAME} BIN_DIR=${EXEDIR} QINFO_VERBOSITY=${QINFO_VERBOSITY} INFO_VERBOSITY=$(INFO_VERBOSITY) VALGRINDLOGFILENAME=${VALGRINDNOINPUTLOG} VALGRINDEXEARGS="")
